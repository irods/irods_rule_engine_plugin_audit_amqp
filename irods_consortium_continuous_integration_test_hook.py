...
def install_messaging_package(message_broker):
    ...
    raise ValueError('unsupported message broker [{}]'.format(message_broker))
...
def main():
    ...
    if options.do_setup:
        ...
        irods_python_ci_utilities.subprocess_get_output(['sudo', '-EH', 'python2', '-m', 'pip', 'install',
        ...
        irods_python_ci_utilities.install_os_packages_from_files(
            glob.glob(os.path.join(os_specific_directory,
                                   'irods-rule-engine-plugin-audit-amqp*.{}'.format(package_suffix))
            )
        )

        irods_python_ci_utilities.subprocess_get_output(['sudo', 'su', '-', 'irods', '-c',
            'python2 scripts/add_audit_rule_engine_to_rule_engines.py'],
            check_rc=True)
    ...
    try:
        irods_python_ci_utilities.subprocess_get_output(['sudo', 'su', '-', 'irods', '-c',
            'python2 scripts/run_tests.py --xml_output --run_s={} 2>&1 | tee {}; exit $PIPESTATUS'.format(test, test_audit_log)],
            check_rc=True)
